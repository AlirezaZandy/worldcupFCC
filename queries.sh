...
echo "$($PSQL "SELECT DISTINCT(name) FROM games FULL JOIN teams ON games.winner_id=teams.team_id OR games.opponent_id=teams.team_id WHERE name ~ '^Co' ORDER BY(name)")"